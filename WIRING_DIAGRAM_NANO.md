...
| **8** | Кнопка розрядів | Кнопка перемикання розрядів (одиниці/десятки/сотні) |
...
### 4. Кнопка перемикання розрядів

```
Кнопка (тактовий вимикач):
├── Вивід 1 → Arduino Nano Pin 8
└── Вивід 2 → Arduino Nano GND
```

**Примітка:**
- Використовується INPUT_PULLUP
- Натискання перемикає режим редагування розрядів:
  - Одиниці (Units) → Десятки (Tens) → Сотні (Hundreds) → Одиниці (циклічно)
- Працює тільки в меню "Set Angle"
- Дозволяє швидко встановити кут, перемикаючись між одиницями, десятками та сотнями

---

### 5. Тумблер перемикання напрямку (CW/CCW)
...
| Кнопка розрядів | 8 | Digital (INPUT_PULLUP) | Перемикання розрядів |
